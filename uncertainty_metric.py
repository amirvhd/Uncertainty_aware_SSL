...
from utils.metrics import OELoss, SCELoss, TACELoss, ACELoss, ECELoss
...
def ensemble(n, nh, lamda1, lamda2, dataset, targets, n_cls, test_loader, dl=False, semi=False, semi_percent=1,
    ...
    if semi == True:
        ...
        for i in range(n):
            ...
            probs_ensemble2_model.append(predict(test_loader, linear_model, laplace=False))
    probs_ensemble2_model = np.array(probs_ensemble2_model)
    print(probs_ensemble2_model.shape)
    probs_ensemble2 = np.mean(probs_ensemble2_model, 0)
    print(probs_ensemble2.shape)
    acc_ensemble2 = (probs_ensemble2.argmax(-1) == targets).mean()
    oe = OELoss()
    sce = SCELoss()
    ace = ACELoss()
    ...
    ece_res = ece.loss(output=probs_ensemble2, labels=targets, logits=True, n_bins=5)
    ece_ensemble2 = ECE(bins=15).measure(probs_ensemble2, targets)
